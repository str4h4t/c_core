...
for meth in methods:
    ...
    for threshold in np.arange(0.75, 1, 0.01):
        pairs_opin = pd.read_csv('vodafone_raw_0.8_0.8_' + meth + '_port_lvl_opin_0921.csv')
        pairs_opout = pd.read_csv('vodafone_raw_0.8_0.8_' + meth + '_port_lvl_opout_0921.csv')
        #print('DCCA')
        TP = 0
        predicted = []
        for row in pairs_opin.iterrows():
            nodes = row[1][1].split(';')
            nodes.sort()
            nodes = '_'.join(nodes)
        ...
        for row in pairs_opout.iterrows():
            nodes = row[1][1].split(';')
            nodes.sort()
            nodes = '_'.join(nodes)
            corr = row[1]['correlation']
        ...
        results.append({'threshold': threshold, 'tp': TP, 'fp': FP, 'fn': FN, 'precision': Precision, 'recall': Recall,
                        'f1_score': fscore})
    pd.DataFrame(results).to_csv('vodafone_raw_lbc_batch_opin_vs_opout_result_port_lvl_0921' + meth + '.csv')
