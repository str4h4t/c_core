...
for meth in methods:
    ...
    for threshold in np.arange(0.75, 1, 0.01):
        pairs = pd.read_csv('vodafone_raw_0.8_0.8_' + meth + '_port_lvl_hawkes_macro_0921.csv')
        #print('DCCA')
        TP = 0
        predicted = []
        ...
        results.append({'threshold': threshold, 'tp': TP, 'fp': FP, 'fn': FN, 'precision': Precision, 'recall': Recall,
                        'f1_score': fscore})
    pd.DataFrame(results).to_csv('vodafone_raw_lbc_batch_result_port_lvl_hawkes_macro_0921_' + meth + '.csv')
